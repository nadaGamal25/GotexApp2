...
export default function ForgetPassword() {
  ...
  async function sendDataToApi(){
    try {
          const response = await axios.post('https://dashboard.go-tex.net/gotex-co-test/user/forget-password-email', theUser);
  ...
  return (
    <>
    <div className="d-flex min-vh-100 bg-light px-3">
      ...
      if(err.context.label ==='email'){
        return <div key={index} className="alert alert-danger my-2">الايميل يجب ان يكون بريدا الكتروني صحيح</div>
            ...
            <Link to="/" className="btn btn-secondary m-1">إلغاء</Link>
            </form>
        </div>
    </div>
    </>
